...
运行：
  python main_v0.2.3-us.py                # 默认监控三环集团（sz300408）
  python main_v0.2.3-us.py 600519         # 启动即监控贵州茅台
  python main_v0.2.3-us.py AAPL           # 启动即监控苹果（美股）
  python main_v0.2.3-us.py -p 9000        # 指定端口
  python main_v0.2.3-us.py --no-browser   # 不自动打开浏览器
...
<body>
  <nav>
    <div class="logo">
      ...
      <span>股票监控</span><span class="ver">v0.2.3</span>
    </div>
  ...
  <script>
    ...
    const state = { data: null, range: 50, ma: [5, 20], pollMs: 5000,
                    pollTimer: null, chart: null, results: [], ki: -1, stockName: '', market: 'sz' };
...
def main():
    parser = argparse.ArgumentParser(description="股票监控 v0.2.3-us（测试版）")
    ...
    print("=" * 56)
    print(f"  股票监控 v0.2.3-us  监控标的: {config['name']} ({code})")
    print(f"  浏览器访问: {url}")
    if config["market"] == "us":
