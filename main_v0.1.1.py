...
功能：
  - 单只股票监控（默认贵州茅台 sh600519，可用命令行参数修改）
...
运行：
  python main.py                # 监控贵州茅台
  python main.py 000001         # 监控平安银行
...
def validate_bar(bar: dict) -> dict | None:
...
class HistoryManager:
    ...
    def _load_cache(self) -> bool:
        ...
        try:
            ...
            bars = [validate_bar(b) for b in df.to_dict("records")]
...
def main():
    ...
    parser.add_argument("symbol", nargs="?", default=DEFAULT_SYMBOL,
                        help="股票代码，如 600519 / 000001 / 600519.SH，默认贵州茅台")
    ...
    if code is None:
        print(f"无效股票代码: {args.symbol!r}（示例: 600519）", file=sys.stderr)
        sys.exit(1)
    config["symbol"] = code
