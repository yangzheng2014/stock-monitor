...
功能：
  ...
  - 个股新闻：右侧面板展示当前标的相关新闻（东方财富搜索，免费接口），可隐藏
  - 涨停/跌停大框：可一键隐藏/展开
  - 收藏：监控页 ☆ 收藏标的，收藏页从主页横向滑动进入（鼠标拖拽 / 触摸滑动）

运行：
  python main_v0.2.4.py                # 默认监控三环集团（sz300408）
  python main_v0.2.4.py 600519         # 启动即监控贵州茅台
  python main_v0.2.4.py -p 9000        # 指定端口
  python main_v0.2.4.py --no-browser   # 不自动打开浏览器
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  @media (max-width: 720px) {
    ...
    #monitor { flex-direction: column; } .limit-panel { width: 100%; max-height: none; }
...
<body>
  <nav>
    <div class="logo">
      ...
      <span>股票监控</span><span class="ver">v0.2.4</span>
    </div>
  ...
  <script>
    ...
    const state = { data: null, range: 50, ma: [5, 20], pollMs: 5000,
                    pollTimer: null, chart: null, results: [], ki: -1,
                    stockName: '', market: 'sz', curCode: '' };
    ...
    function initPanelToggles() {
      if (localStorage.getItem('lp_hidden') === '1') {
    ...
    function saveFavs(fs) { localStorage.setItem('favs', JSON.stringify(fs)); }
    function isFav(code) { return getFavs().some(function (f) { return f.code === code; }); }
    function updateFavBtn() {
      const on = isFav(state.curCode);
      $('fav-btn').textContent = on ? '★' : '☆';
      $('fav-btn').classList.toggle('on', on);
    }
    function toggleFav() {
      const fs = getFavs();
      const cur = { code: state.curCode, name: state.stockName, market: state.market };
      const idx = fs.findIndex(function (f) { return f.code === cur.code; });
      if (idx >= 0) fs.splice(idx, 1); else fs.push(cur);
      saveFavs(fs);
      updateFavBtn();
      renderFavs();
    }
    function renderFavs() {
      const fs = getFavs();
      $('fav-count').textContent = fs.length ? fs.length + ' 只' : '';
      if (!fs.length) {
        $('fav-list').innerHTML = '<div class="empty">暂无收藏，在监控页点击 ☆ 收藏股票</div>';
        return;
      }
      $('fav-list').innerHTML = fs.map(function (f, i) {
        return '<div class="fitem" data-i="' + i + '">' +
          '<span class="f-name">' + f.name + '</span>' +
          '<span class="f-code">' + f.code + '</span>' +
          '<span class="f-mkt">' + f.market.toUpperCase() + '</span></div>';
      }).join('');
      $('fav-list').querySelectorAll('.fitem').forEach(function (el) {
        el.addEventListener('click', function () {
          const f = getFavs()[+el.dataset.i];
          if (f) { selectStock(f); setPage(0); }
        });
      });
    }
    ...
    function setPage(i) {
      pageIdx = Math.max(0, Math.min(1, i));
...
def main():
    parser = argparse.ArgumentParser(description="股票监控 v0.2.4（测试版）")
